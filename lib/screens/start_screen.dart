...
class _StartScreenState extends State<StartScreen>
  ...
  static const _backgroundAsset = 'assets/images/start_background.png';
              ...
              child: ScaleTransition(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    SizedBox(width: size.shortestSide * 0.06),
                    FantasyPuzzleButton(
                      size: size.shortestSide * 0.2,
                      onPressed: _onPuzzle,
                    ),
...
/// Silberner Puzzle-Button im gleichen Fantasy-Stil.
class FantasyPuzzleButton extends StatefulWidget {
  const FantasyPuzzleButton({
    super.key,
    required this.onPressed,
    this.size = 88,
  });
  ...
  @override
  State<FantasyPuzzleButton> createState() => _FantasyPuzzleButtonState();
}

class _FantasyPuzzleButtonState extends State<FantasyPuzzleButton> {
  bool _pressed = false;
      ...
      child: GestureDetector(
        ...
        child: AnimatedScale(
          ...
          child: SizedBox(
            ...
            height: size,
            child: CustomPaint(
              painter: _FantasyPuzzlePainter(pressed: _pressed),
            ),
          ),
        ),
      ),
...
class _FantasyPuzzlePainter extends CustomPainter {
  _FantasyPuzzlePainter({required this.pressed});
  ...
  void paint(Canvas canvas, Size size) {
    ...
    _drawInnerWell(canvas, center, radius);
    _drawGems(canvas, center, radius, size);
    _drawPuzzleIcon(canvas, center, radius);
    _drawCrestCrystal(canvas, center, radius, size);
  }
  ...
  void _drawPuzzleIcon(Canvas canvas, Offset center, double radius) {
    final pieceSize = radius * 0.22;
    final gap = radius * 0.04;
    final positions = [
      Offset(-pieceSize / 2 - gap / 2, -pieceSize / 2 - gap / 2),
      Offset(pieceSize / 2 + gap / 2, -pieceSize / 2 - gap / 2),
      Offset(-pieceSize / 2 - gap / 2, pieceSize / 2 + gap / 2),
      Offset(pieceSize / 2 + gap / 2, pieceSize / 2 + gap / 2),
    ];
    ...
    for (final offset in positions) {
      final pieceCenter = center + offset;
      ...
      canvas.drawRRect(
        RRect.fromRectAndRadius(rect, Radius.circular(radius * 0.04)),
        Paint()
          ..shader = LinearGradient(
            begin: Alignment.topLeft,
            end: Alignment.bottomRight,
  ...
  @override
  bool shouldRepaint(covariant _FantasyPuzzlePainter oldDelegate) {
    return oldDelegate.pressed != pressed;
  }
}
